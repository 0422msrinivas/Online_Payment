...
Class Action {
	...
	function save_student(){
		...
		// Check if the id_no already exists
    ...
    $id = intval($id);  // Ensures it's an integer
	...
	function save_proctor(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO proctor set $data");
		}else{
			$save = $this->db->query("UPDATE proctor set $data where id = $id");
		}
		if($save)
			return 1;
	}

function delete_proctor() {
    extract($_POST);

    // Validate and sanitize input
    $proctor_id = intval($id);  // Ensure it's an integer

    // Use prepared statement to avoid SQL injection
    $stmt = $this->db->prepare("DELETE FROM proctor WHERE id = ?");
...
// Removed duplicate save_student_proctor function to resolve redeclaration error.



	function save_student_proctor() {
    extract($_POST);
    include 'db_connect.php';

    // Optional: Check if already exists for same student
    $check = $conn->query("SELECT * FROM student_proctor WHERE student_id = '$student_id' AND id != '$id'");
    if ($check->num_rows > 0) {
    ...
    if (empty($id)) {
        $save = $conn->query("INSERT INTO student_proctor SET $data");
    } else {
        $save = $conn->query("UPDATE student_proctor SET $data WHERE id = '$id'");
    }
...
function delete_student_proctor() {
    extract($_POST);

    // Validate and sanitize input
    $student_proctor_id = intval($id);  // Ensure it's an integer

    // Use prepared statement to avoid SQL injection
    $stmt = $this->db->prepare("DELETE FROM student_proctor WHERE id = ?");
    $stmt->bind_param("i", $student_proctor_id);
